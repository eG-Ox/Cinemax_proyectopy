...
from dao.sala_dao import SalaConFuncionesError, SalaDAO, SalaNoEncontradaError
...
def crear_sala(datos: SalaCrear):
    sala = dao.insertar(Sala(datos.nombre_sala, datos.capacidad))
    return sala.to_dict()
...
def actualizar_sala(sala_id: int, datos: SalaActualizar):
    try:
        sala = dao.actualizar(sala_id, datos.nombre_sala, datos.capacidad)
        return sala.to_dict()
